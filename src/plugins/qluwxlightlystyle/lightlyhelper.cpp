...
namespace Lightly
{
    ...
    void Helper::renderButtonFrame(
        ...
        const bool hasFocus, const bool sunken, const  bool mouseOver, const bool enabled, const bool windowActive, const AnimationMode mode, const qreal opacity ) const
     {
        ...
        if( isDarkTheme( palette ) && enabled )
    ...
    void Helper::renderCheckBox(
        ...
        bool sunken, const bool mouseOver, CheckBoxState state, CheckBoxState target, const bool windowActive, qreal animation ) const
    {
        // setup painter
        painter->setRenderHint( QPainter::Antialiasing, true );
        painter->setPen( Qt::NoPen );
        ...
        QColor background ((state == CheckOn || state == CheckPartial) ? palette.color( QPalette::Highlight ) : palette.color( QPalette::Button ));
        ...
        if( darkTheme )
            topHighlight( painter, frameRect, radius );
    ...
    {
        static auto platformName = QGuiApplication::platformName();
        static const bool s_isWayland = platformName.startsWith(QLatin1String("wayland"), Qt::CaseInsensitive);
        return s_isWayland;
    }
    ...
    {
        static auto platformName = QGuiApplication::platformName();
        static const bool s_isWindows = platformName.startsWith(QLatin1String("windows"), Qt::CaseInsensitive);
        return s_isWindows;
    }
